...
let tempMaps = {"maps":[
    ...
    {"uuid":"F2:AE:DB:70:B1:B6","uuid_ios":"39A56CD5-D5BC-E4AE-1693-E71F472670B3","x":12,"y":80,"name":"demo_point","route":2,"map_status":"D"},
]}
    ...
    bluetooth.startScanning({
        ...
        onDiscovered: function (peripheral) {
            ...
            walkMap(peripheral.UUID,peripheral.RSSI,function(cb){
                ...
                logData.uuid_ios = pageData.uuid
                logData.uuid = ""
...
function genMap(UUID,RSSI){
    ...
    if(Object.keys(pageData.map).length !== 0){
        ...
        httpModule.getImage("http://202.129.16.68/~offtduac/bgMaps/r"+route+".jpg").then((r) => {
            ...
            var path = fs.path.join(folder.path, "r"+route+".png");
            appSettings.setString("bgMaps", path)
            mapLayout.backgroundImage = "http://202.129.16.68/~offtduac/bgMaps/r"+route+".jpg"
        }, (e) => {
            console.log(e)
        });    
